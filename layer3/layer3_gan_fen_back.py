# Pix2Pix GAN architecture used here for background layer of fencing
...
def load_images(size=(width, height)):
    ...
    for i in range(batch):
        ...
        for j in range(frames):
            ...
            pixels = Image.fromarray(pred_ref_back[i,j,:,:,:])
...
filename = '../../data/maps_fen_back.npz'
...
def summarize_performance(step, g_model, dataset, n_samples=3):
	...
	filename2 = '../../models_3/model_fen_back.h5'
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	for i in range(n_steps):
		...
		print('fen_back>%d, d1[%.3f] d2[%.3f] g[%.3f]' % (i+1, d_loss1, d_loss2, g_loss))
		# summarize model performance
		if (i+1) % (10) == 0:
			summarize_performance(i, g_model, dataset)

# load image data
dataset = load_real_samples('../../data/maps_fen_back.npz')
