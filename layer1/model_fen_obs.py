...
vid2 = np.load('fencing-vid2.npy')[0:1000,:,:,:]
print(inp.shape)
...
# back.fit(X_train, y_train, batch_size=50, epochs=100, validation_data=(X_test, y_test), shuffle=True, callbacks=callbacks)

# obs = feature_extractor_and_layer_flow_estimator()
# obs.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
obs = load_model("obs_fen.hdf5")
X_train, X_test, y_train, y_test = train_test_split(inp, vid2, test_size=0.2, random_state=42)
print(X_train.shape, y_train.shape)
checkpoint = ModelCheckpoint("obs_fen.hdf5", monitor='loss', verbose=1,save_best_only=True, mode='auto', period=10)
callbacks = [checkpoint]
obs.fit(X_train, y_train, batch_size=50, epochs=150, validation_data=(X_test, y_test), shuffle=True, callbacks=callbacks)
